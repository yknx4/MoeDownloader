...
public:
	...
	static void DownloadFiles(Object^ data){//function to Parse Html Tags and call download
		...
		String^ HtmlContent=DanbooruDownloader::ReadDanbooru(Parameteres->page,Parameteres->tags); //Get Raw HTML with needed parameters
		String^ TempUrl; //Initialize string to store URL
		HtmlNodeCollection^ nodos_a = DanbooruDownloader::GetHtmlNodes(HtmlContent,"//a[@class='directlink largeimg'] | //a[@class='directlink smallimg']");; //Select all nodes with direct image link
	...
	static array<String^>^ GetDownloadLinks(int page, array<String^>^tags) //Function to get RAW html source code from site with 'page' and 'tags' as source
	{
		String^ FinalUrl= gcnew String("http://danbooru.donmai.us/post?page=");//URL String initialization
		FinalUrl += page.ToString() + "&commit=Search&tags=";//Add page and "&tags=" to URL
		for each (String^ tag in tags)//Cycle to add each tag to URL
